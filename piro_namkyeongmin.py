...
# 이 줄부터 코드를 작성해주세요!
import random, time
# random 모듈 -> 랜덤으로 숫자를 생성하는 모듈
# time 모듈 -> time.sleep()으로 의도적인 지연시간을 주는 모듈, 이때 ()안의 값은 초단위를 사용함
#####################################################################################
...
class Character:
    def __init__(self, name, hair, clothes, shoes):
        ...
        self.shoes=shoes
        # 'self.속성=값' 구조로 캐릭터 각각의 속성에 매개변수로 받은 값 할당함
        ###############################################################
...
class DetectiveGame:
    def __init__(self):
        ...
        self.murderer = None
        self.suspect = None
        self.dying_message = None
        self.detective_name = None 
        self.clues=[] # 리스트 자료형을 가지는 self.clues
        self.lives=2
        # 속성별 초기값 세팅하기
        #####################################################################################
    ...
    def show_intro(self):
        ...
        # 이 줄부터 코드를 작성해주세요!
        print("탐정님의 성함을 알려주세요: ", end="")
        self.detective_name = input().strip()
        print(f"\n{self.detective_name} 탐정님, 반갑습니다! 마음껏 피로그래밍 22기 해커톤을 즐겨주세요:)\n")
        time.sleep(1)
        # 입력받은 탐정의 이름을 포함해 메세지 출력함
        ##################################################################

        ##### 5. 게임의 캐릭터 중 한 명을 랜덤으로 희생자로 지정하고, 희생자로 지정된 캐릭터를 목록에서 제거해주세요! #############

        # 이 줄부터 코드를 작성해주세요!
        self.victim = random.choice(self.characters) # 랜덤으로 희생자 설정함
        self.characters.remove(self.victim)
        # 희생자를 찾아 리스트에서 삭제 -> 추후 murder로 설정되지 않기 위함+용의자 목록에 나타나지 않기 위함
        ###################################################################################################
        ...
        if dying_message_type == "hair":
            self.dying_message= "머리스타일은 {0} 윽..☠️".format(self.murderer.hair)
        elif dying_message_type == "clothes":
            self.dying_message= "옷은 {0} 윽..☠️".format(self.murderer.clothes)
        elif dying_message_type == "shoes":
            self.dying_message= "신발은 {0} 윽..☠️".format(self.murderer.shoes)
        # dying_message_type에 따라 적합한 dying_message 할당함(match_dying_message의 문자와 동일하게 맞춤)
        ######################################################################################################
        ...
        # 이 줄부터 코드를 작성해주세요!
        print(f"\n문제의 노트북 주위에 있는 사람은 {len(self.characters)}명입니다.")
        # len()으로 characters list의 요소 개수를 셈
        ########################################################
        time.sleep(1)
    ...
    def investigate(self):
        ...
        for char in self.characters:
            if char.name == choice_name:
                # 조사하려는 용의자가 characters 리스트에 존재한다면,
                self.suspect = char
                # 리스트에 존재하는 용의자와 동일한 이름을 속성 suspect로 저장함
                print(f"\n{self.suspect.name}의 인상착의를 봅니다.")
                print(f"- 머리 : {self.suspect.hair}")
                print(f"- 옷 : {self.suspect.clothes}")
                print(f"- 신발 : {self.suspect.shoes}")
                # 조사한 용의자의 인상착의를 출력함
                
                self.clues.append({
                "name": self.suspect.name,
                "hair": self.suspect.hair,
                "clothes": self.suspect.clothes,
                "shoes":self.suspect.shoes
                })
                # clues 리스트에 조사한 용의자의 이름과 인상착의 저장함
                return
                # play 함수의 다음 명령(prompt_choice)을 실행하기 위해 리턴값을 반환함
        #####################################################################################
    ...
    def accuse(self):
        ...
        # 이 줄부터 코드를 작성해주세요!
        for index, char in enumerate(self.characters, 1): # enumerate(리스트, 시작인덱스)를 이용해 리스트의 요소를 인덱스와 함께 출력함
            print(f"{index}. {char.name}")
        self.clue_list() # 용의자 조사 시 clues 리스트에 저장한 용의자 정보를 조회하는 함수를 불러옴
        ###########################################################################################
...
----------------------------------------------------------------------------------"""
    ...
    def reset_game(self):
        self.__init__()
        # 직접 수정한 부분
        # 다시 실행했을 때, 이전 판에서 victim 제거한 characters 리스트를 초기화하기 위하여 아예 __init__() 함수를 호출함
